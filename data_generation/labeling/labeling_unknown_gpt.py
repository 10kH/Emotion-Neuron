...
# 입력 및 출력 파일 경로 설정
output_file = os.environ.get('OUTPUT_FILE', '/home/woody/workspace/Emotion-Neuron/data/synth_gemini_step4_10_labeled_gpt.json')

# 백업 디렉토리 설정
backup_dir = os.environ.get('BACKUP_DIR', '/home/woody/workspace/Emotion-Neuron/data/backup_gpt')
os.makedirs(backup_dir, exist_ok=True)

# 현재 시간으로 타임스탬프 생성
timestamp = datetime.now().strftime('%Y%m%d%H%M%S')
backup_file = os.path.join(backup_dir, f'synth_gemini_step4_10_labeled_gpt_backup_{timestamp}.json')
...
# 'unknown' 또는 'error'으로 레이블링된 대화만 선택
dialogues_to_relabel = [
    dialogue for dialogue in labeled_dialogues 
    if dialogue.get("gpt-4o-mini") in ["unknown", "error"]
]
total_to_relabel = len(dialogues_to_relabel)

if total_to_relabel == 0:
    print("재레이블링할 'unknown' 또는 'error' 감정을 가진 대화가 없습니다.")
    exit()

print(f"재레이블링할 'unknown' 또는 'error' 감정을 가진 대화 수: {total_to_relabel}")

# tqdm 진행바 설정
pbar = tqdm(total=total_to_relabel, desc="감정 재레이블링 진행 중", unit="대화")

# 재레이블링이 필요한 대화들만 순회
for dialogue in dialogues_to_relabel:
    # 프롬프트 작성 (논문에 실을 만큼 전문적이고 정제된 형태)
    prompt = f"""
You are a sentiment analysis expert. Analyze the following conversation and determine the primary emotion that best represents the overall sentiment expressed throughout the dialogue.
...
"""
    ...
    try:
        # OpenAI GPT API를 사용하여 감정 예측
        response = openai.ChatCompletion.create(
            model="gpt-4o-mini",
            temperature=0.3,  # 응답의 일관성을 높이기 위해 낮은 온도 사용
            max_tokens=10,     # 짧은 응답을 유도
            messages=[
    ...
    # 주기적으로 백업 생성 (예: 매 100개마다)
    if pbar.n % 100 == 0:
        current_timestamp = datetime.now().strftime('%Y%m%d%H%M%S')
        current_backup_file = os.path.join(backup_dir, f'synth_gemini_step4_10_labeled_gpt_backup_{current_timestamp}.json')
        try:

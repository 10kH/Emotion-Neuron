...
# 백업 디렉토리 설정
backup_dir = os.environ.get('BACKUP_DIR', '/home/woody/workspace/Emotion-Neuron/data/backup_gemini')
os.makedirs(backup_dir, exist_ok=True)

# 현재 시간으로 타임스탬프 생성
timestamp = datetime.now().strftime('%Y%m%d%H%M%S')
backup_file = os.path.join(backup_dir, f'synth_gemini_step4_10_labeled_gemini_backup_{timestamp}.json')
...
for target_label in relabeling_targets:
    ...
    dialogues_to_relabel = [
        ...
        if dialogue.get("gemini-1.5-flash") == target_label
    ]
...
"""
        ...
        try:
            ...
            response = model.generate_content(
                ...
                generation_config=genai.types.GenerationConfig(
                    temperature=0.3,  # 감정 예측의 일관성을 높이기 위해 낮은 온도 사용
                    max_output_tokens=10  # 짧은 응답을 유도
                ),
                safety_settings={
        ...
        if processed_count % BACKUP_INTERVAL == 0:
            ...
            current_backup_file = os.path.join(backup_dir, f'synth_gemini_step4_10_labeled_gemini_backup_{current_timestamp}.json')
            try:
...
final_backup_file = os.path.join(backup_dir, f'synth_gemini_step4_10_labeled_gemini_backup_final_{final_timestamp}.json')
try:
...
print("모든 재레이블링 작업이 성공적으로 완료되었습니다.")
